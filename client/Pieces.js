...
const tp = require("./torrent_utils");

const is2DArrayTrue = (arr) => {
  return arr.every((l1) => l1.every(Boolean) == true);
};
...
const Pieces = class {
  ...
  addRequested(pieceBlock) {
    this._requested[pieceBlock.index][pieceBlock.begin / tp.BLOCK_LEN] = true;
  }

  addReceived(pieceBlock) {
    this._received[pieceBlock.index][pieceBlock.begin / tp.BLOCK_LEN] = true;
  }

  needed(pieceBlock) {
    if (is2DArrayTrue(this._requested)) {
  ...
  isDone() {
    return is2DArrayTrue(this._received);
  }
  ...
  printPercentDone() {
    ...
    for (const x of Array(100).keys()) {
      var progress =
        all_status.slice(x * blockSize, (x + 1) * blockSize).filter(Boolean)
          .length / all_status.slice(x * blockSize, (x + 1) * blockSize).length;
